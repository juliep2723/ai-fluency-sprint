...
function Insp3Content() {
    ...
    const handleSubmit = async (e: React.FormEvent, formLocation: string) => {
        ...
        try {
            ...
            if (response.ok) {
                router.push('/sidekick_insp3/thank-you')
            }
        } catch (error) {
            ...
            <section id="hero-capture" className="relative min-h-[60vh] md:min-h-[55vh] flex flex-col justify-start pt-32 pb-8 px-6 overflow-hidden">
                ...
                >
                    <motion.h1
                        ...
                        variants={fadeInUp}
                    >
                        Which daily disaster do you need to fix first?
                    </motion.h1>

                    {!selectedDisaster ? (
                        <motion.div className="flex flex-col gap-4 max-w-3xl mx-auto mb-6" variants={fadeInUp}>
                            <Button
                                onClick={() => setSelectedDisaster('neighbor')}
                                size="lg"
                                className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-8 py-6 rounded-xl backdrop-blur-sm shadow-xl w-full transition-all whitespace-normal h-auto leading-tight text-left flex flex-col items-start gap-2 group"
                            >
                                <span className="text-xl md:text-2xl font-bold group-hover:text-teal transition-colors">
                                    😤 A pushy neighbor or co-worker.
                                </span>
                                <span className="text-base md:text-lg font-medium text-gray-300">
                                    You need to say no—clearly—without starting a war.
                                </span>
                            </Button>

                            <Button
                                onClick={() => setSelectedDisaster('spreadsheet')}
                                size="lg"
                                className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-8 py-6 rounded-xl backdrop-blur-sm shadow-xl w-full transition-all whitespace-normal h-auto leading-tight text-left flex flex-col items-start gap-2 group"
                            >
                                <span className="text-xl md:text-2xl font-bold group-hover:text-teal transition-colors">
                                ...
                                <span className="text-base md:text-lg font-medium text-gray-300">
                                    Looks like it was organized by a raccoon, and you need to use it in 5 minutes.
                                </span>
                            </Button>

                            <Button
                                onClick={() => setSelectedDisaster('document')}
                                size="lg"
                                className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-8 py-6 rounded-xl backdrop-blur-sm shadow-xl w-full transition-all whitespace-normal h-auto leading-tight text-left flex flex-col items-start gap-2 group"
                            >
                                <span className="text-xl md:text-2xl font-bold group-hover:text-teal transition-colors">
                                ...
                                <span className="text-base md:text-lg font-medium text-gray-300">
                                    Written in legalese. You need to know what matters in 90 seconds.
                                </span>
                            </Button>
                        </motion.div>
                    ) : (
                        ...
                        >
                            ...
                            <motion.form
                                onSubmit={(e) => handleSubmit(e, `hero_top_${selectedDisaster}`)}
                            ...
                            >
                                <input
                                    ...
                                    placeholder="Enter your email for all 7 prompts"
                                    required
...
export default function SidekickInsp3Page() {
    return (
